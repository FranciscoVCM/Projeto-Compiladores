...
static int child_count(struct node *n) {
    int count = 0; struct node_list *c;
...
static void free_holder_only(struct node *n) { if (!n) return; if (n->children) free(n->children); free(n); }
static struct node *build_block_from_holder(struct node *holder) {
    int meaningful = meaningful_child_count(holder);
    struct node *result;
    if (meaningful == 0) { result = newnode(Block, NULL); free_holder_only(holder); return result; }
    if (meaningful == 1) { result = first_meaningful_child(holder); holder->children->next = NULL; free_holder_only(holder); return result; }
    result = newnode(Block, NULL); append_meaningful_children(result, holder); free_holder_only(holder); return result;
}

void yyerror(char *s) {
    syntax_errors = 1;
    if (only_errors) return;
    if (suppress_errors) return;
    if (token_line == last_lex_error_line) return;
    if (yychar == 0 && syntax_error_count > 0) return;
...
program: CLASS IDENTIFIER LBRACE class_body RBRACE {
...
class_body:
    { $$ = make_holder(); }
  | class_body field_decl { append_holder($1, $2); free_holder_only($2); $$ = $1; }
  | class_body method_decl { addchild($1, $2); $$ = $1; }
  | class_body SEMICOLON { $$ = $1; }
  | class_body error SEMICOLON { yyerrok; $$ = $1; }
  ;

field_decl: PUBLIC STATIC type field_ids SEMICOLON {
    struct node_list *child;
    $$ = make_holder();
...
field_ids:
    IDENTIFIER { $$ = make_holder(); addchild($$, newnode(Identifier, $1)); }
  | field_ids COMMA IDENTIFIER { addchild($1, newnode(Identifier, $3)); $$ = $1; }
  ;

method_decl: PUBLIC STATIC method_header method_body {
...
/*
 * method_header is expanded to avoid the formal_params epsilon rule causing a
 * $default reduce in the parser. Without the epsilon, when an invalid token
 * appears after LPAR, the parser can correctly use the "LPAR error RPAR"
 * recovery rule instead of escalating to class_body level.
 */
method_header:
...
param_list:
    param_decl { $$ = newnode(MethodParams, NULL); addchild($$, $1); }
  | param_list COMMA param_decl { addchild($1, $3); $$ = $1; }
  ;
...
method_body_items:
    { $$ = make_holder(); }
  | method_body_items var_decl { append_holder($1, $2); free_holder_only($2); $$ = $1; }
  | method_body_items stmt { if (!is_empty_block($2)) addchild($1, $2); $$ = $1; }
  ;

var_decl: type var_ids SEMICOLON {
    struct node_list *child;
    $$ = make_holder();
    ...
    free_ast($1); free_ast($2);
} ;

var_ids:
    IDENTIFIER { $$ = make_holder(); addchild($$, newnode(Identifier, $1)); }
  | var_ids COMMA IDENTIFIER { addchild($1, newnode(Identifier, $3)); $$ = $1; }
  ;

stmt:
    LBRACE stmt_list RBRACE { $$ = build_block_from_holder($2); }
  | IF LPAR expr RPAR stmt %prec LOWER_THAN_ELSE {
        $$ = newnode(If, NULL);
        addchild($$, $3); addchild($$, $5); addchild($$, newnode(Block, NULL));
    }
  ...
  | IDENTIFIER ASSIGN expr SEMICOLON {
        $$ = newnode(Assign, NULL);
        addchild($$, newnode(Identifier, $1)); addchild($$, $3);
    }
  ...
  | error SEMICOLON { yyerrok; $$ = newnode(Block, NULL); }
  ;

stmt_list:
    { $$ = make_holder(); }
  | stmt_list stmt { if (!is_empty_block($2)) addchild($1, $2); $$ = $1; }
  ;
...
or_expr:
    or_expr OR and_expr { $$ = newnode(Or, NULL); addchild($$, $1); addchild($$, $3); }
  | and_expr
  ;

and_expr:
    and_expr AND xor_expr { $$ = newnode(And, NULL); addchild($$, $1); addchild($$, $3); }
  | xor_expr
  ;

xor_expr:
    xor_expr XOR eq_expr { $$ = newnode(Xor, NULL); addchild($$, $1); addchild($$, $3); }
  | eq_expr
  ;

eq_expr:
    eq_expr EQ rel_expr { $$ = newnode(Eq, NULL); addchild($$, $1); addchild($$, $3); }
  | eq_expr NE rel_expr { $$ = newnode(Ne, NULL); addchild($$, $1); addchild($$, $3); }
  | rel_expr
  ;

rel_expr:
    rel_expr LT shift_expr { $$ = newnode(Lt, NULL); addchild($$, $1); addchild($$, $3); }
  | rel_expr GT shift_expr { $$ = newnode(Gt, NULL); addchild($$, $1); addchild($$, $3); }
  | rel_expr LE shift_expr { $$ = newnode(Le, NULL); addchild($$, $1); addchild($$, $3); }
  | rel_expr GE shift_expr { $$ = newnode(Ge, NULL); addchild($$, $1); addchild($$, $3); }
  | shift_expr
  ;

shift_expr:
    shift_expr LSHIFT add_expr { $$ = newnode(Lshift, NULL); addchild($$, $1); addchild($$, $3); }
  | shift_expr RSHIFT add_expr { $$ = newnode(Rshift, NULL); addchild($$, $1); addchild($$, $3); }
  | add_expr
  ;

add_expr:
    add_expr PLUS mul_expr { $$ = newnode(Add, NULL); addchild($$, $1); addchild($$, $3); }
  | add_expr MINUS mul_expr { $$ = newnode(Sub, NULL); addchild($$, $1); addchild($$, $3); }
  | mul_expr
  ;

mul_expr:
    mul_expr STAR unary_expr { $$ = newnode(Mul, NULL); addchild($$, $1); addchild($$, $3); }
  | mul_expr DIV unary_expr { $$ = newnode(Div, NULL); addchild($$, $1); addchild($$, $3); }
  | mul_expr MOD unary_expr { $$ = newnode(Mod, NULL); addchild($$, $1); addchild($$, $3); }
  | unary_expr
  ;

unary_expr:
    NOT unary_expr { $$ = newnode(Not, NULL); addchild($$, $2); }
  | MINUS unary_expr { $$ = newnode(Minus, NULL); addchild($$, $2); }
  | PLUS unary_expr { $$ = newnode(Plus, NULL); addchild($$, $2); }
  | primary_expr
  ;
...
expr_list:
    expr { $$ = make_holder(); addchild($$, $1); }
  | expr_list COMMA expr { addchild($1, $3); $$ = $1; }
  ;

parse_args: PARSEINT LPAR IDENTIFIER LSQ expr RSQ RPAR {
