...
stmt_core:
    LBRACE { if (!hard_suppress_errors) suppress_errors = 0; } stmt_list RBRACE {
        if (suppress_errors && !hard_suppress_errors)
            pending_error_after_block = 1;
        if (!hard_suppress_errors)
            suppress_errors = 0;
        $$ = build_block_from_holder($3);
    }
  ...
  | RETURN { if (!hard_suppress_errors) suppress_errors = 0; } expr SEMICOLON {
        $$ = newnode(Return, NULL);
        addchild($$, $3);
    }
