  ...
  | method_body_items var_decl {
    ...
    | method_body_items stmt {
        ...
        if (!hard_suppress_errors && !pending_error_after_block) {
            /* só limpar supressão quando já não estamos a sair
               de uma recuperação por statement inválido */
            if (!suppress_errors)
                suppress_errors = 0;
        }
...
stmt_entry:
    {
        ...
        /* se acabámos de recuperar de um "public ..." inválido,
           e o próximo statement também começa por PUBLIC,
           não queremos repetir erro imediatamente */
        if (suppress_errors && yychar != PUBLIC)
            suppress_errors = 0;
